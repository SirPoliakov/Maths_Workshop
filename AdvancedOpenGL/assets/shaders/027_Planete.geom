...
void make_face(vec3 a, vec3 b, vec3 c)
{
    ...
    vec4 face_color = colorChange * (mat3(proj_matrix*mv_matrix) * face_normal).z;

    gl_Position = proj_matrix * mv_matrix * vec4(a, 1.0);
    gs_out.color = face_color;
    EmitVertex();

    gl_Position = proj_matrix * mv_matrix * vec4(b, 1.0);
    gs_out.color = face_color;
    EmitVertex();

    gl_Position = proj_matrix * mv_matrix * vec4(c, 1.0);
    gs_out.color = face_color;
    EmitVertex();
